...
- 백그라운드 영역에서 서버와 통신하여, 그 결과를 웹 페이지의 일부분만 표시할 수 있다.
...
- 테이터에 접근하거나 화면 구성을 동적으로 조작하기 위해 사용되는 DOM 모델
- 데이터 교환을 위한 JSON이나 XML
- 웹 서버와의 비동기 통신을 위한 XMLHttpRequest객체
- 위에서 언급한 모든 기술을 결합하여 사용자가 작업 흐름을 제어하는 데 사용되는 자바 스크립트
...
- Ajax에서XMLHttpRequest 객체는 웹 브라우저가 서버와 데이터를 교환할 때 사용된다.
...
```
- Ajax 요청 시 XMLHttpRequest 객체가 각 상태로 readyState가 변한다.
처음에는 readyState가 0(객체명.UNSENT, 보내지 않음)이었다가,
open 메서드를 호출하는 순간 1(객체명.OPEND)로 바뀌고 send 메서드를 
호출하게 되면 순차적으로 2(객체명.HEADERS_RECEIVED), 3(객체명.LOADING),
4(객체명.DONE)가 된다면 상태코드(객체명.STATUS)가 성공을 의미하는 200, 201일 때 응답을 확인해야 한다.