...
그림에서 버튼에 입장에서 버튼을 감싸고 있는 레이아웃은 부모 레이아웃(Parent Layout)이라고 부른다. 그리고 버튼은 상하좌우 각각 연결점을 가지고 있다. 만약 버튼에 연결점이 부모 레이아웃에 왼쪽 벽면에 연결하면 부모 레이아웃은 버튼에 타깃(Target)이 된다. 이렇게 연결점과 타깃이 연결되면 연결 선이 만들어지고 파랗게 표시된다.  또 한 버튼이 어디에 위치하는지 알기 위해서는 적어도 왼쪽과 위쪽에 연결되어 있어야 한다. 즉 부모 레이아웃의 왼쪽과 위쪽에 연결되면 제약 조건이 완성된다.
...
```
그다음 입력되어 있는 코드가 화면 전체를 감싸고 있는 레이아웃이다. Component Tree 창의 계층에서 가장 위쪽에 있고 이를 최상위 레이아웃이라고 부른다. 여기에서는 ConstraintLayout은 이 태그의 이름으로 사용되고 그 앞에는 패키지 이름(androidx.constraintlayout.wid-get)이 사용된다. 만약 레이아웃이 안드로이드 기본 API에 포함되어 있다면 그 위젯이나 레이아웃의 이름만 입력하면 된다. 하지만 외부 라이브러리에서 불러온 것이라면 패키지 이름까지 같이 입력해야 된다. (ConstraintLayout은 안드로이드 SDK에 나중에 추가되면서 외부 라이브러리로 분류되어 있다) 
