...
# 자바 코드에서 화면 구성
MainActivity.java 파일을 열면 setContentView() 메서드를 호출하는 부분이있다. setContentView()는 activity_main.xml 파일을 파라미터로 전달하면 이 레이아웃 파일이 액티비티라 불리는 소스파일과 연결된다. 이 두개의 파일이 연결되어 하나의 화면을 만들게 되는 것이다.
